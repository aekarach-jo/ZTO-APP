...
## 5. สถานะฝั่งแอป: รองรับไว้แล้ว ✅

แอป **parse ฟิลด์ `billNo` / `paymentNo` / `amount` ไว้เรียบร้อยแล้ว** ทั้ง 3 endpoint
หน้าใบเสร็จทำเสร็จแล้วเช่นกัน — พอ backend เริ่มส่งค่ามา จะขึ้นทันทีโดยไม่ต้องแก้แอปอีก

ระหว่างที่ยังไม่ส่ง:
- แถว Bill No. / Payment No. จะถูก **ซ่อน** (ไม่ขึ้นแถวว่าง)
- barcode จะ fallback ไปใช้ `order.id` แทน (ยังสแกนได้ แต่ไม่ใช่เลขบิลที่ตรงกับหลังบ้าน)

---

## 6. ผลลัพธ์ฝั่งแอปหลังได้ฟิลด์นี้
